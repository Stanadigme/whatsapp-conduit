...
let runtimePoll = 0;
function updateRuntimeView(state) { const connected = state.connection === 'connected'; document.querySelectorAll('[data-requires-connection]').forEach(element => { element.hidden = !connected; }); $('offline-card').hidden = connected; if (connected) { baileysPairingRequested = false; baileysPairingError = ''; } }
...
async function refreshBaileysLinkQr() { clearTimeout(baileysLinkQrPoll); try { const state = await api('/api/pairing/baileys/status'); const waiting = state.status === 'waiting_qr'; if (waiting) baileysPairingError = ''; const button = $('baileys-link-start'); button.disabled = baileysPairingRequested || waiting; $('baileys-link-status').textContent = waiting ? 'QR disponible — scannez-le dans WhatsApp → Réglages → Appareils connectés.' : baileysPairingRequested ? 'Préparation du QR de ré-appairage…' : baileysPairingError || 'Aucune session d’appairage active.'; $('baileys-link-status').className = baileysPairingError ? 'error' : 'muted'; $('baileys-link-qr').hidden = !waiting; if (waiting) $('baileys-link-qr').src = '/api/pairing/baileys/qr.svg?t=' + Date.now(); } catch (error) { $('baileys-link-status').textContent = 'État du QR indisponible.'; $('baileys-link-status').className = 'error'; $('baileys-link-qr').hidden = true; } finally { baileysLinkQrPoll = setTimeout(refreshBaileysLinkQr, 2000); } }
