...
let runtimePoll = 0;
function updateRuntimeView(state) { const connected = state.connection === 'connected'; document.querySelectorAll('[data-requires-connection]').forEach(element => { element.hidden = !connected; }); $('offline-card').hidden = connected; $('baileys-link-card').hidden = connected; }
