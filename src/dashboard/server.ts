...
let runtimePoll = 0;
function updateRuntimeView(state) { const connected = state.connection === 'connected'; document.querySelectorAll('[data-requires-connection]').forEach(element => { element.hidden = !connected; }); $('offline-card').hidden = connected; if (connected) baileysPairingRequested = false; }
...
async function refreshBaileysLinkQr() { clearTimeout(baileysLinkQrPoll); try { const state = await api('/api/pairing/baileys/status'); const waiting = state.status === 'waiting_qr'; const button = $('baileys-link-start'); button.disabled = baileysPairingRequested || waiting; $('baileys-link-status').textContent = waiting ? 'QR disponible — scannez-le dans WhatsApp → Réglages → Appareils connectés.' : baileysPairingRequested ? 'Préparation du QR de ré-appairage…' : 'Aucune session d’appairage active.'; $('baileys-link-status').className = 'muted'; $('baileys-link-qr').hidden = !waiting; if (waiting) $('baileys-link-qr').src = '/api/pairing/baileys/qr.svg?t=' + Date.now(); } catch (error) { $('baileys-link-status').textContent = 'État du QR indisponible.'; $('baileys-link-status').className = 'error'; $('baileys-link-qr').hidden = true; } finally { baileysLinkQrPoll = setTimeout(refreshBaileysLinkQr, 2000); } }
$('baileys-link-qr').onerror = () => { $('baileys-link-qr').hidden = true; };
$('baileys-link-start').onclick = async () => { if (!window.confirm('Le linked device actuel sera remplacé. Continuer ?')) return; baileysPairingRequested = true; try { await api('/api/pairing/baileys/start', { method: 'POST' }); await refreshBaileysLinkQr(); } catch (error) { baileysPairingRequested = false; $('baileys-link-status').textContent = error.message; $('baileys-link-status').className = 'error'; await refreshBaileysLinkQr(); } };
$('connect').onclick = () => { token = $('token').value; $('auth').textContent = 'Jeton conservé uniquement en mémoire.'; $('auth').className = 'muted'; refresh(); void refreshRuntimeView(); void refreshBaileysLinkQr(); api('/api/pairing/status').then(updatePairingControls).catch(showError); };
...
const STYLES_CSS = `:root{color-scheme:light dark;font-family:system-ui,sans-serif}body{margin:0;background:#f4f5f7;color:#1f2933}main{max-width:960px;margin:0 auto;padding:24px}header{margin-bottom:20px}.card{background:white;border:1px solid #d9dee5;border-radius:10px;padding:18px;margin:14px 0;box-shadow:0 1px 2px #0001}h1,h2{margin-top:0}label{display:flex;gap:12px;align-items:center}input,select,button{font:inherit;padding:8px;border:1px solid #bbc4cf;border-radius:6px}button{cursor:pointer;background:#155eef;border-color:#155eef;color:#ffffff;font-weight:600}button:hover{background:#004eeb;border-color:#004eeb}button:focus-visible{outline:3px solid #84adff;outline-offset:2px}.toolbar{display:flex;gap:8px;margin-bottom:12px}.toolbar input{flex:1}.chat{display:flex;gap:12px;align-items:center;justify-content:space-between;border-top:1px solid #e5e7eb;padding:12px 0}.chat div{display:flex;flex-direction:column}.chat small{color:#68737d}.chat span{font-size:.9em;color:#68737d}.qr{background:#fff;color:#000;font:12px/1 monospace;overflow:auto;padding:12px;white-space:pre}.pairing-qr{display:block;box-sizing:border-box;width:min(100%,480px);margin:12px auto;background:#fff;padding:12px}.pairing-qr[hidden]{display:none}.muted{color:#68737d}.error{color:#b42318}@media(prefers-color-scheme:dark){body{background:#111827;color:#e5e7eb}.card{background:#1f2937;border-color:#374151}.chat{border-color:#374151}.qr{background:#fff;color:#000}input,select{background:#111827;color:#e5e7eb;border-color:#4b5563}}`;
...
function bootstrap() { const jid = conversationJid(); if (jid) { void showConversation(jid); return; } void refresh(); void refreshRuntimeView(); void refreshBaileysLinkQr(); api('/api/pairing/status').then(updatePairingControls).catch(showError); }
