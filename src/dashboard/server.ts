...
function historyLimitLabel(job) { if (job.errorCode === 'no_anchor' || job.completionReason === 'no_local_anchor') return ' Aucun message local ne permet encore de demander un lot antérieur.'; if (job.completionReason === 'window_already_delivered') return ' WhatsApp a déjà livré ces messages à cet appareil et ne les renvoie jamais ; pour remonter plus loin, un nouvel appairage est nécessaire.'; if (job.status === 'completed' && !job.coverageComplete) return ' Profondeur maximale fournie par WhatsApp atteinte.'; return ''; }
